...
#define ACTIVATE_POST_BUFFER_LEN (255)
    ...
    offset += sprintf(buffer + offset, ",\"cadVer\":\"%s\",\"cdVer\":\"%s\",\"t\":%d}", CAD_VER, CD_VER, (int) timestamp);
                                     ...
                                     atop_base_response_t *response)
{
    ...
    default:
        snprintf(buffer, ATOP_DEFAULT_POST_BUFFER_LEN, "{\"type\":\"[\\\"timezone\\\",\\\"rateRule\\\"]\",\"t\": %lu }",
                 timestamp);
        break;
    }
...
int atop_service_upgrade_status_update_v41(const char *id, const char *key, int channel, int status)
{
    ...
    uint32_t timestamp = tal_time_get_posix();
    buffer_len = snprintf(buffer, ATOP_DEFAULT_POST_BUFFER_LEN, "{\"type\":%d,\"upgradeStatus\":%d,\"t\":%lu}", channel,
                          status, timestamp);
    PR_DEBUG("POST JSON:%s", buffer);
...
int atop_service_put_rst_log_v10(const char *id, const char *key, const char *rst_buffer)
{
    ...
    uint32_t timestamp = tal_time_get_posix();
    buffer_len = snprintf(buffer, ATOP_DEFAULT_POST_BUFFER_LEN, "{%s,\"t\":%lu}", rst_buffer, timestamp);
    PR_DEBUG("POST JSON:%s", buffer);
...
#define UPDATE_PROPERTY_BUFFER_LEN 255
    ...
    // {"countryCode":"86","phone":"15656065877"}
    buffer_len =
        snprintf(buffer, UPDATE_PROPERTY_BUFFER_LEN, "{\"metas\":{\"catIccId\":\"%s\"},\"t\":%lu}", iccid, timestamp);
    PR_DEBUG("POST JSON:%s", buffer);
